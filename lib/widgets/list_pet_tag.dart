...
import 'package:project/screens/shop/checkout_pet_tag_screen.dart';
import 'package:project/screens/spa_booking/service_selection.dart';
import 'package:project/widgets/utils.dart';
import 'package:shimmer/shimmer.dart';

class PetTagCarousel extends StatefulWidget {
  final List<Map<String, String>> imgArray;

  const PetTagCarousel({
    super.key,
    required this.imgArray,
  });

  @override
  _PetTagCarouselState createState() => _PetTagCarouselState();
}

class _PetTagCarouselState extends State<PetTagCarousel> {
  int _current = 0;
    ...
    return CarouselSlider(
      items: widget.imgArray
          .map((item) => GestureDetector(
                onTap: () {
                  Utils.navigateTo(context, CheckoutPetTagScreen(petTagId: int.parse(item['id'] ?? '0')));
                },
                child: Container(
                  child: Column(
                    children: [
